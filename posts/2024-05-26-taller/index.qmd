...
calendario <- as_tibble(calendario) %>%
  select(semana, fecha1, fecha2) %>% 
  mutate(sesion = paste("sesión", semana))

calendario %>% select(c(sesion, fecha1, fecha2)) %>% 
  mutate(pres_1 = format(date(fecha1), "%A %d %B"),
         pres_2 = format(date(fecha2), "%A %d %B")) %>% 
 flextable() %>%
 colformat_date(j = 2:3, )
